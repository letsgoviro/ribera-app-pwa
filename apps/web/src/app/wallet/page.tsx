...
import { Ticket as TicketIcon, Calendar, MapPin, X, QrCode, CheckCircle, XCircle, Loader2, Share2, Download, Send, AlertCircle } from 'lucide-react'
  ...
  return (
    <motion.div
      ...
      className={`bg-surface-800 border rounded-3xl overflow-hidden transition-all ${
        isUsed || isCancelled ? 'border-surface-600 opacity-60' : 'border-surface-600 active:scale-[0.98]'
      }`}
      onClick={() => !isUsed && !isCancelled && onView(ticket)}
    >
      {/* Ticket header */}
      ...
      <AnimatePresence>
        {selectedTicket && (
          <motion.div
            ...
            onClick={() => { setSelectedTicket(null); setTransferView(false); setTransferMsg(null) }}
          >
            <motion.div
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
